...
class HourGlass(nn.Module):
    def __init__(self, num_modules, depth, num_features, w_o_SE):
        ...
        self.w_o_SE = w_o_SE
        self._generate_network(self.depth, self.w_o_SE)
        
    def _generate_network(self, level, w_o_SE):
        self.add_module('b1_' + str(level), ConvBlock(256, 256, w_o_SE))
        self.add_module('b2_' + str(level), ConvBlock(256, 256, w_o_SE))

        if level > 1:
            self._generate_network(level - 1, w_o_SE)
        else:
            self.add_module('b2_plus_' + str(level), ConvBlock(256, 256, w_o_SE))

        self.add_module('b3_' + str(level), ConvBlock(256, 256, w_o_SE))
...
class EmoNet(nn.Module):
    def __init__(self, num_modules=1, n_expression=128, n_blocks=1, 
        ...
        self.conv3 = ConvBlock(128, 128, w_o_SE)
        self.conv4 = ConvBlock(128, 256, w_o_SE)

        for hg_module in range(self.num_modules):
            self.add_module('m' + str(hg_module), HourGlass(1, 2, 256, w_o_SE=w_o_SE))
            self.add_module('top_m_' + str(hg_module), ConvBlock(256, 256, w_o_SE))
